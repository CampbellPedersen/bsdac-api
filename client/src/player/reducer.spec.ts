...
describe('reducer', () => {
  ...
  it('when rap selected > should return state with selected rap', () => {
    const state: AudioPlayerState = { };
    const newState = playerReducer(state, { type: 'RapSelected', rap });

    expect(newState).toEqual({ rap });
  });

  it('when audio stream requested > should return loading state', () => {
    const state: AudioPlayerState = { streamUrl: 'https://rap.local' };
    const newState = playerReducer(state, { type: 'AudioStreamRequested' });

    expect(newState).toEqual({ streamUrl: undefined });
  });
  it('when audio stream received > should return state with url', () => {
    ...
    expect(newState).toEqual({ streamUrl: url });
  });
});